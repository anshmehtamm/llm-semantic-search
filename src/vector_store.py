...
class VectorStore:
    ...
    def __init__(self, dimension, index_file=config.INDEX_FILE, id_map_file=config.ID_MAP_FILE):
        ...
        self.index = None
        self.id_map = {} # Maps index position (0, 1, 2...) to actual product_id
    ...
    def build_index(self, embeddings: np.ndarray, product_ids: list):
        ...
        # IndexFlatL2: Exact search, good for smaller datasets (<1M vectors maybe), slow for large N.
        # For larger datasets, consider approximate nearest neighbor (ANN) indexes like:
        # - IndexIVFFlat: Faster, requires training, needs tuning (nlist).
        # - IndexHNSWFlat: State-of-the-art ANN, fast, high memory usage, no training needed.
        # Example for IVFFlat:
        ...
        # self.index.train(embeddings) # Training step for IVFFlat
    ...
    def _save_index(self):
        ...
        if self.index is not None:
            try:
                ...
                # Save the index
                faiss.write_index(self.index, str(self.index_file))  # Convert Path to string
        ...
        if self.id_map:
            # Ensure the directory exists for the ID map file too
            os.makedirs(os.path.dirname(self.id_map_file), exist_ok=True)
            save_json(self.id_map, str(self.id_map_file))  # Convert Path to string
        else:
    ...
    def _load_index(self):
        ...
        if index_exists and map_exists:
            ...
            try:
                ...
                loaded_map_str_keys = load_json(str(self.id_map_file))
                if loaded_map_str_keys:
                    ...
                    self.index = None # Invalidate index if map loading fails
