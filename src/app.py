...
try:
    ...
    # Import data_loader ONLY if you need to load product details for display
...
# --- Load Search Engine ---
# Use Streamlit's caching to load the engine only once
@st.cache_resource
def load_engine():
...
# This might be slow if the product catalog is large. Consider using a database
# or a smaller lookup file for the demo.
@st.cache_data
def load_product_details():
    logger.info("Loading product details for display...")
    # !!! Adapt this to load necessary details (title, maybe URL/image) based on product_id
    # This is a placeholder using the same loader as for embeddings, which might be inefficient
    try:
        # Load only ID and title/description for display
        # Adjust the loading function or use a separate, lighter file/DB query
        ...
        return products_df.to_dict('index') # Faster lookup by product_id
            ...
            if results_ids:
                ...
                for pid, score in zip(results_ids, results_scores):
                    ...
                    results_data.append({
                        ...
                        "Similarity Score": f"{1 - score:.4f}" if score is not None else "N/A" # Example: Convert L2 distance to a similarity score (higher is better)
...
# Add more info or controls to the sidebar if needed 